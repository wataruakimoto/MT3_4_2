...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (isRotate) {
			...
			pendulum.angularAcceleration = -(9.8f / pendulum.length) * sin(pendulum.angle);
		...
		sphere.center.x = pendulum.anchor.x + sin(pendulum.angle) * pendulum.length;
		sphere.center.y = pendulum.anchor.y - cos(pendulum.angle) * pendulum.length;
